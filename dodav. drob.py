...
class FractionVisualizerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Тренажер (5 клас): Додавання дробів з однаковими знаменниками")
        try:
            self.state('zoomed')
        except tk.TclError:
            self.attributes('-zoomed', True)

        self.MAX_DENOMINATOR = 20  # Для 5 класу зменшено макс. знаменник для зручності
    ...
    def _create_fraction_controls(self, parent, title, num_var, den_var, col):
        ...
        den_widgets = self._create_slider_unit(frame, "Знаменник (спільний):", den_var)
    ...
    def _adjust_value(self, var, delta):
        ...
        self._on_slider_change(None, var)

    def _on_slider_change(self, value=None, var=None):
        if var is not None and value is not None: var.set(int(float(value)))

        # Для 5 класу: синхронізуємо знаменники, щоб вони завжди були однакові!
        if var == self.den1_var:
            self.den2_var.set(self.den1_var.get())
        elif var == self.den2_var:
            self.den1_var.set(self.den2_var.get())

        if self.den1_var.get() < 1: self.den1_var.set(1); self.den2_var.set(1)
    ...
    def _generate_new_task(self):
        # Генеруємо ТІЛЬКИ ОДИН знаменник для обох дробів
        d = random.randint(3, 15)
        n1 = random.randint(1, d - 1)
        n2 = random.randint(1, d - 1)
        self._load_state((n1, d, n2, d))
    ...
    def _load_state(self, state):
        ...
        self._on_slider_change(var=self.den1_var)
    ...
    def visualize(self):
        ...
        # Перевірка (оскільки знаменники завжди однакові, логіка дуже проста)
        if den1 > 0 and den1 == self.task_d1:
            user_n = num1 + num2
            correct_n = self.task_n1 + self.task_n2

            if user_n == correct_n:
                # Перевіряємо, чи може дріб бути скорочений (для п'ятикласників)
                if math.gcd(user_n, den1) > 1:
                    self.success_var.set("✔ Правильно! Але дріб ще можна скоротити ;)")
                else:
    ...
    def _build_solution_for_task(self):
        """Спрощене пояснення за підручником 5 класу"""
        n1, d = self.task_n1, self.task_d1
        n2 = self.task_n2
        self.solution_steps = []
        ...
        # Перевірка на неправильний дріб (виділення цілої частини)
        if sum_n > d:
            whole, rem = divmod(sum_n, d)
            self.solution_steps.append(("bold", "--- КРОК 3: ВИДІЛЕННЯ ЦІЛОЇ ЧАСТИНИ ---"))
            ...
            if rem == 0:
                ...
                self.solution_steps.append(("bold", f"Кінцева відповідь: {whole} цілих і {rem}/{d}"))
        elif sum_n == d:
            self.solution_steps.extend([
                ("bold", "--- КРОК 3: ЦІЛЕ ЧИСЛО ---"),
                ("normal", f"Чисельник дорівнює знаменнику, отже це дорівнює 1."),
                ("bold", "Кінцева відповідь: 1")
            ])
        else:
