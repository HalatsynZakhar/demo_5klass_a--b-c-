...
class NaturalsApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="Натуральні числа (§ 1-3)", bg=ACCENT, fg=WHITE, font=("Segoe UI", 24, "bold")).pack(side="left", padx=30)
    ...
    def show_intro(self):
        ...
        tk.Label(f, text="Що таке натуральні числа?", font=("Segoe UI", 36, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 30))
    ...
    def show_comparison(self):
        ...
        tk.Label(f, text="Порівняння натуральних чисел", font=("Segoe UI", 36, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 30))
        
        rules = tk.Frame(f, bg=WHITE, bd=2, relief="solid")
        rules.pack(fill="x", pady=20, ipadx=20, ipady=20)
        
        tk.Label(rules, text="Правила порівняння:", font=("Segoe UI", 24, "bold"), bg=WHITE, fg=ACCENT).pack(anchor="w")
        tk.Label(rules, text="1. З двох натуральних чисел більше те, у записі якого більше цифр.\n   (Наприклад: 1024 > 999)\n\n2. Якщо кількість цифр однакова, порівнюємо розряди зліва направо.\n   (Наприклад: 5432 > 5399, бо 4 сотні > 3 сотні)", 
                 font=("Segoe UI", 20), bg=WHITE, fg=TEXT, justify="left").pack(anchor="w", pady=10)

        # Interactive quick check
        self.cmp_frame = tk.Frame(f, bg=BG)
        self.cmp_frame.pack(pady=40)
    ...
    def gen_cmp_task(self):
        ...
        tk.Label(self.cmp_frame, text=f"{a}", font=("Courier New", 48, "bold"), bg=BG).pack(side="left", padx=20)
        
        btn_f = tk.Frame(self.cmp_frame, bg=BG)
        btn_f.pack(side="left", padx=20)
        ...
        tk.Button(btn_f, text="<", font=("Arial", 36, "bold"), command=lambda: check("<")).pack(side="left", padx=10)
        tk.Button(btn_f, text=">", font=("Arial", 36, "bold"), command=lambda: check(">")).pack(side="left", padx=10)
        
        tk.Label(self.cmp_frame, text=f"{b}", font=("Courier New", 48, "bold"), bg=BG).pack(side="left", padx=20)
        
        res = tk.Label(self.cmp_frame, text="", font=("Segoe UI", 24, "bold"), bg=BG, width=15)
        res.pack(side="left", padx=20)
    ...
    def show_rounding(self):
        ...
        task_box.pack(pady=40)

        self.lbl_instruction = tk.Label(task_box, text="", bg=BG, fg=MUTED, font=("Segoe UI", 20), justify="center")
        ...
        self.btn_hl = tk.Button(task_box, text="💡 Показати розряд", bg=WHITE, fg=ORANGE, font=("Segoe UI", 14, "bold"),
                                relief="flat", bd=0, cursor="hand2", pady=6, highlightbackground="#fde68a", highlightthickness=1,
                                command=self._toggle_highlight)
        self.btn_hl.pack(pady=(10, 0))

        # Input Box
        inp_box = tk.Frame(left, bg=BG)
        inp_box.pack(pady=20)

        tk.Label(inp_box, text="Твоя відповідь:", bg=BG, fg=MUTED, font=("Segoe UI", 16)).pack()
        ...
        self.lbl_display = tk.Label(self.display_frame, text="", bg=WHITE, fg=TEXT, font=("Segoe UI", 48, "bold"), width=10, anchor="e")
        self.lbl_display.pack()

        self.lbl_feedback = tk.Label(inp_box, text="", bg=BG, font=("Segoe UI", 18, "bold"), justify="center")
        ...
        self.btn_ok = tk.Button(left, text="✓ Перевірити", bg=ACCENT, fg=WHITE, font=("Segoe UI", 18, "bold"),
                                relief="flat", bd=0, cursor="hand2", pady=15, command=self._check)
        self.btn_ok.pack(pady=20, ipadx=50)

        self.btn_next = tk.Button(left, text="▶ Наступне", bg=GREEN, fg=WHITE, font=("Segoe UI", 18, "bold"),
                                  relief="flat", bd=0, cursor="hand2", pady=15, command=self._new_task)
        ...
        # Правило
        tk.Label(rpad, text="Правило округлення", bg=PANEL, fg=MUTED, font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=(0,10))
        rule_box = tk.Frame(rpad, bg="#eff6ff", highlightbackground="#bfdbfe", highlightthickness=1)
        rule_box.pack(fill="x", ipadx=10, ipady=10)
        ...
        tk.Label(rule_box, text=rule_text, bg="#eff6ff", fg=TEXT, font=("Segoe UI", 14), justify="left", anchor="w").pack(fill="x", padx=10)

        tk.Frame(rpad, bg=BORDER, height=2).pack(fill="x", pady=20)

        # Числова пряма
        self.btn_line = tk.Button(rpad, text="📏 Показати на числовій прямій", bg=WHITE, fg=ACCENT, font=("Segoe UI", 14, "bold"),
                                  relief="flat", bd=0, cursor="hand2", pady=10, highlightbackground=ACCENT, highlightthickness=1,
                                  command=self._toggle_line)
        self.btn_line.pack(fill="x")

        self.line_canvas = tk.Canvas(rpad, bg=PANEL, highlightthickness=0, height=250)

        # Score
        self.lbl_score_round = tk.Label(rpad, text="Рахунок: 0 / 0", font=("Segoe UI", 18, "bold"), bg=PANEL, fg=MUTED)
        self.lbl_score_round.pack(side="bottom", pady=20)
    ...
    def _check(self):
        ...
        self.btn_next.pack(pady=20, ipadx=50)
    ...
    def _draw_number_line(self):
        ...
        while v <= draw_hi:
            ...
            c.create_text(x, ya+35, text=str(v), fill=TEXT, font=("Segoe UI", 12, "bold"))
            v += step
        ...
        c.create_text(x_half, ya+25, text=str(half), fill=MUTED, font=("Segoe UI", 10))

        # Original number
        xn = to_x(number)
        c.create_oval(xn-8, ya-8, xn+8, ya+8, fill=DOT_ORIG, outline="")
        c.create_text(xn, ya-30, text=task["number_str"], fill=DOT_ORIG, font=("Segoe UI", 14, "bold"))
        ...
        # Legend
        c.create_oval(margin, 170, margin+12, 182, fill=DOT_ORIG, outline="")
        c.create_text(margin+20, 176, text="вихідне число", fill=DOT_ORIG, font=("Segoe UI", 12), anchor="w")
        
        c.create_oval(margin+150, 170, margin+162, 182, fill=DOT_ROUND, outline="")
        c.create_text(margin+170, 176, text="округлене", fill=DOT_ROUND, font=("Segoe UI", 12), anchor="w")
