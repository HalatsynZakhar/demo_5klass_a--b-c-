...
class FractionVisualizerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Тренажер (5 клас): Віднімання мішаних чисел")
        try:
            self.state('zoomed')
        except tk.TclError:
        ...
        self.task_total_n1 = 0
        self.task_total_n2 = 0
        self.task_d = 1
        self.correct_total_n = 0
        ...
        # Змінні ТІЛЬКИ для відповіді користувача
        ...
        # Єдиний блок керування - введення відповіді
    ...
    def _generate_new_task(self):
        ...
        # Запобігаємо від'ємним результатам
        if w1 == w2 and n1 < n2:
            n1, n2 = n2, n1
        ...
        self._load_state((total_n1, total_n2, d))

    def _load_state(self, state):
        self._set_controls_state(tk.NORMAL)
        self.result_status_var.set("")
        total_n1, total_n2, d = state
        self.task_total_n1, self.task_total_n2, self.task_d = total_n1, total_n2, d

        # Правильна відповідь у неправильному дробі (для внутрішньої перевірки)
        self.correct_total_n = total_n1 - total_n2

        # Скидаємо повзунки відповіді. Знаменник ставимо як у завданні
    ...
    def _draw_task_text(self):
        ...
        def draw_frac(total_n, d, x):
            if d == 0: return x
            whole, frac_n = divmod(total_n, d)
            ...
            if frac_n > 0 or (whole == 0 and total_n >= 0):
                n_str, d_str = str(frac_n), str(d)
                n_w, d_w = task_font.measure(n_str), task_font.measure(d_str)
                max_w = max(n_w, d_w) + 10
                self.task_canvas.create_text(x + max_w / 2, canvas_h / 2 - 16, text=n_str, font=task_font,
                                             anchor="center")
                self.task_canvas.create_line(x, canvas_h / 2, x + max_w, canvas_h / 2, width=3)
                self.task_canvas.create_text(x + max_w / 2, canvas_h / 2 + 16, text=d_str, font=task_font,
                                             anchor="center")
                x += max_w

            return x + 20

        x_pos = draw_frac(self.task_total_n1, self.task_d, x_pos)
        self.task_canvas.create_text(x_pos, canvas_h / 2, text="-", font=font_whole, anchor="center")
        x_pos += 30
        draw_frac(self.task_total_n2, self.task_d, x_pos)
    ...
    def visualize(self):
        ...
        w1, f_n1 = divmod(self.task_total_n1, self.task_d)
        w2, f_n2 = divmod(self.task_total_n2, self.task_d)

        ax_title1.set_title(self.format_user_input_title("Зменшуване", w1, f_n1, self.task_d), fontsize=18)
        ax_title2.set_title(self.format_user_input_title("Від'ємник", w2, f_n2, self.task_d), fontsize=18)
        ...
        self._draw_overlapping_circles(ax1, self.task_total_n1, self.task_d, self.color1)
        self._draw_overlapping_circles(ax2, self.task_total_n2, self.task_d, self.color2)

        user_total_n = user_w * user_d + user_n
        if user_w == 0 and user_n == 0 and self.correct_total_n != 0:
            self.draw_placeholder(ax3, "Введіть\nрезультат")
        else:
            self._draw_overlapping_circles(ax3, user_total_n, user_d, 'mediumseagreen')
    ...
    def _check_user_answer(self):
        ...
        if user_d != self.task_d:
            self.result_status_var.set("Пам'ятайте: при відніманні знаменник залишається тим самим!")
            self.result_status_label.config(style="Error.TLabel")
            return

        # Перевірка на математичну правильність
        if user_total_n == self.correct_total_n:
            if user_n >= user_d:
                self.result_status_var.set(
                    "Правильно пораховано! Але виділіть цілу частину (чисельник більший за знаменник).")
                self.result_status_label.config(style="Success.TLabel")
            else:
    ...
    def _build_solution_for_task(self):
        w1, n1 = divmod(self.task_total_n1, self.task_d)
        w2, n2 = divmod(self.task_total_n2, self.task_d)
        d = self.task_d
        ...
        self.solution_steps.append(
            ("normal", "Цілі віднімаємо від цілих, чисельники — від чисельників.\nЗнаменник залишаємо без змін."))

        # Якщо чисельник першого дробу менший, треба позичати
        if n1 < n2:
            self.solution_steps.append(("bold", "--- КРОК 1: 'ПОЗИЧАННЯ' ОДИНИЦІ ---"))
            self.solution_steps.append(("normal",
                                        f"Чисельник {n1} менший за {n2}, тому відняти не можемо.\nПозичаємо 1 у цілої частини ({w1}):\n1 = {d}/{d}."))
            new_w1 = w1 - 1
            new_n1 = n1 + d
            self.solution_steps.append(
                ("normal", f"Отримуємо замість першого дробу: {new_w1} і ({n1} + {d})/{d}\n-> {new_w1} {new_n1}/{d}"))
            w1, n1 = new_w1, new_n1
        else:
            self.solution_steps.append(("bold", "--- КРОК 1: ВІДНІМАННЯ ---"))

        diff_w = w1 - w2
        diff_n = n1 - n2

        if w1 > 0 or w2 > 0:
            self.solution_steps.append(("normal", f"1. Віднімаємо цілі: {w1} - {w2} = {diff_w}"))

        self.solution_steps.append(("normal", f"2. Віднімаємо чисельники: {n1} - {n2} = {diff_n}"))
        self.solution_steps.append(("normal", f"Знаменник залишається {d}."))

        if diff_w > 0:
            if diff_n > 0:
                self.solution_steps.append(("bold", f"Кінцева відповідь: {diff_w} {diff_n}/{d}"))
            else:
                self.solution_steps.append(("bold", f"Кінцева відповідь: {diff_w}"))
        else:
            if diff_n > 0:
