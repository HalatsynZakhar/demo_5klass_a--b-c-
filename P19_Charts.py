...
class ChartsApp(tk.Tk):
    ...
    def show_intro(self):
        ...
        tk.Label(f, text="Діаграми", font=("Segoe UI", 32, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 10))
        tk.Label(
            f,
            text=(
    ...
    def show_line(self):
        ...
        tk.Label(f, text="Лінійна діаграма", font=("Segoe UI", 30, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 10))
        tk.Label(
            f,
            text=(
        ...
        self.line_msg = tk.Label(top, text="", font=("Segoe UI", 16, "bold"), bg=BG, fg=MUTED)
        self.line_msg.pack(side="left")
        tk.Button(top, text="🎲 Нові дані", font=("Segoe UI", 14, "bold"), bg=ACCENT, fg=WHITE, bd=0, padx=16, pady=10, command=self._line_new).pack(
            side="right", padx=10
        )

        self.line_canvas = tk.Canvas(f, bg=WHITE, height=360, bd=2, relief="ridge")
    ...
    def show_bar(self):
        ...
        tk.Label(f, text="Стовпчаста діаграма", font=("Segoe UI", 30, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 10))
        tk.Label(
            f,
            text=(
        ...
        top = tk.Frame(f, bg=BG)
        top.pack(fill="x", pady=10)
        self.bar_msg = tk.Label(top, text="", font=("Segoe UI", 16, "bold"), bg=BG, fg=MUTED)
        self.bar_msg.pack(side="left")
        tk.Button(top, text="🎲 Нові дані", font=("Segoe UI", 14, "bold"), bg=ACCENT, fg=WHITE, bd=0, padx=16, pady=10, command=self._bar_new).pack(
            side="right", padx=10
        )

        self.bar_canvas = tk.Canvas(f, bg=WHITE, height=360, bd=2, relief="ridge")
    ...
    def show_reading(self):
        ...
        tk.Label(f, text="Читання діаграм", font=("Segoe UI", 30, "bold"), bg=BG, fg=TEXT).pack(pady=(0, 10))
        tk.Label(
            f,
        ...
        top = tk.Frame(f, bg=BG)
        top.pack(fill="x", pady=10)
        tk.Button(top, text="🔁 Інша діаграма", font=("Segoe UI", 14, "bold"), bg=ACCENT, fg=WHITE, bd=0, padx=16, pady=10, command=self._reading_toggle).pack(side="right", padx=10)
        self.reading_title = tk.Label(top, text="", font=("Segoe UI", 16, "bold"), bg=BG, fg=MUTED)
        self.reading_title.pack(side="left")

        self.reading_canvas = tk.Canvas(f, bg=WHITE, height=320, bd=2, relief="ridge")
        self.reading_canvas.pack(fill="x", pady=10, padx=10)

        self.quiz_box = tk.Frame(f, bg=WHITE, bd=2, relief="solid", padx=20, pady=15)
        self.quiz_box.pack(fill="x", padx=10, pady=10)
        self.quiz_q = tk.Label(self.quiz_box, text="", font=("Segoe UI", 16, "bold"), bg=WHITE, fg=TEXT, wraplength=self.SW - 200, justify="left")
        self.quiz_q.pack(anchor="w")
        self.quiz_btns = tk.Frame(self.quiz_box, bg=WHITE)
        self.quiz_btns.pack(pady=12)
        self.quiz_msg = tk.Label(self.quiz_box, text="", font=("Segoe UI", 16, "bold"), bg=WHITE)
    ...
    def show_trainer(self):
        ...
        self.task_canvas = tk.Canvas(left, bg=WHITE, height=280, bd=2, relief="ridge")
    ...
    def _draw_scale_formula(self, cv, unit, value):
        ...
        cx = w // 2
        cv.create_text(cx, 70, text=f"{unit} одиниць = 1 мм", font=("Segoe UI", 18, "bold"), fill=MUTED)
        cv.create_text(cx, 120, text=f"значення {value} : {unit} = ?", font=("Segoe UI", 22, "bold"), fill=TEXT)
        cv.create_line(cx - 240, 155, cx + 240, 155, width=2, fill=BORDER)
        h = 160
        cv.create_rectangle(cx - 35, h, cx + 35, h + 80, fill=SKY, outline=WHITE, width=2)
        cv.create_text(cx, h + 40, text="мм", font=("Segoe UI", 16, "bold"), fill=WHITE)
    ...
    def _draw_line_diagram(self, cv, data, unit_name, title, show_value_labels=True):
        ...
        top = 60
        n = max(1, len(data))
        avail = max(160, h - top - 50)
        row_h = max(44, min(68, int(avail / n)))

        max_val = max(v for _, v in data) if data else 1
        scale = (right - left) / max(1, max_val)

        if title:
            cv.create_text(30, 25, text=title, font=("Segoe UI", 14, "bold"), fill=TEXT, anchor="w")
        cv.create_text(w - 30, 25, text=unit_name, font=("Segoe UI", 12, "bold"), fill=MUTED, anchor="e")
        ...
        if ticks > 0:
            ...
            for t in range(0, max_val + 1, axis_step):
                ...
                cv.create_text(x, top - 28, text=str(t), font=("Segoe UI", 10, "bold"), fill=MUTED)

        for i, (name, val) in enumerate(data):
            y = top + i * row_h + 30
            cv.create_text(30, y, text=name, font=("Segoe UI", 14, "bold"), fill=TEXT, anchor="w")
            x2 = left + val * scale
            cv.create_line(left, y, x2, y, width=10, fill=INDIGO)
            cv.create_oval(x2 - 8, y - 8, x2 + 8, y + 8, fill=ORANGE, outline=WHITE, width=2)
            if show_value_labels:
                cv.create_text(x2 + 12, y, text=str(val), font=("Segoe UI", 12, "bold"), fill=MUTED, anchor="w")
        return axis_step
    ...
    def _draw_bar_diagram(self, cv, data, unit_name, step, title, show_value_labels=True):
        ...
        for t in range(0, max_tick + 1, step):
            ...
            cv.create_line(left - (10 if major else 6), y, left, y, width=2 if major else 1, fill=TEXT if major else BORDER)
            cv.create_line(left, y, right, y, width=1, fill=BORDER)
            if major:
                cv.create_text(left - 12, y, text=str(t), font=("Segoe UI", 10, "bold"), fill=MUTED, anchor="e")

        n = len(data) if data else 1
        bar_area = right - left - 20
        bar_w = max(30, min(90, int(bar_area / n) - 20))
        ...
        for name, val in data:
            ...
            cv.create_text(x + bar_w / 2, bottom + 10, text=name, font=("Segoe UI", 11, "bold"), fill=TEXT, anchor="n", width=bar_w + 10)
            if show_value_labels:
                cv.create_text(x + bar_w / 2, bottom - h - 8, text=str(val), font=("Segoe UI", 11, "bold"), fill=MUTED, anchor="s")
            x += bar_w + gap
