...
class FractionVisualizerApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Тренажер (5 клас): Додавання мішаних чисел")
        try:
            self.state('zoomed')
        except tk.TclError:
        ...
        self.correct_improper_n, self.correct_d = 0, 1
        ...
        # Єдина панель для вводу відповіді
        self.ans_controls = self._create_fraction_controls(controls_frame, "Введіть вашу відповідь:",
                                                           self.ans_w_var, self.ans_n_var, self.ans_d_var, 1)
    ...
    def _create_fraction_controls(self, parent, title, whole_var, num_var, den_var, col):
        ...
        ttk.Separator(frame, orient="horizontal").pack(pady=10, fill="x", expand=True)
    ...
    def _adjust_value(self, var, delta):
        ...
        if var == self.ans_d_var and new_val < 1:
            new_val = 1
        elif var != self.ans_d_var and new_val < 0:
            new_val = 0
        var.set(new_val)
    ...
    def _on_slider_change(self, value=None, var=None):
        ...
        if self.ans_d_var.get() == 0: self.ans_d_var.set(1)
    ...
    def _generate_new_task(self):
        # Генеруємо завдання суто для 5 класу (однакові знаменники)
        ...
        # Щоб не було 0 + 0
        if w1 == 0 and w2 == 0 and n1 == 0 and n2 == 0:
            w1 = 1

        total_n1 = w1 * d + n1
        total_n2 = w2 * d + n2

        self._load_state((total_n1, d, total_n2, d))
    ...
    def _load_state(self, state):
        ...
        # Встановлюємо значення повзунків "Відповіді" в 0 (знаменник ставимо такий самий як у завданні для зручності)
        self.ans_w_var.set(0)
        self.ans_n_var.set(0)
        self.ans_d_var.set(d1)

        # Обчислюємо правильну відповідь у вигляді неправильного дробу
        self.correct_improper_n = n1 + n2
        self.correct_d = d1
    ...
    def _draw_task_fractions(self, n1, d1, n2, d2):
        ...
        def draw_frac(n, d, x):
            ...
            if frac_n > 0 or (whole == 0 and n >= 0 and d != 0):
    ...
    def _set_controls_state(self, state):
        for key in ['whole', 'num', 'den']:
            ...
            if self.ans_controls[key]['scale']:
    ...
    def visualize(self):
        ...
        w1, f_n1 = divmod(self.task_n1, self.task_d1)
        w2, f_n2 = divmod(self.task_n2, self.task_d2)

        ax_title1.set_title(self.format_user_input_title("Перший доданок", w1, f_n1, self.task_d1), fontsize=18)
        ax_title2.set_title(self.format_user_input_title("Другий доданок", w2, f_n2, self.task_d2), fontsize=18)
        ...
        self._draw_overlapping_circles(ax1, self.task_n1, self.task_d1, self.color1)
        self._draw_overlapping_circles(ax2, self.task_n2, self.task_d2, self.color2)

        # Малюємо результат користувача
    ...
    def _check_user_answer(self):
        ...
        # Перехресне множення для перевірки еквівалентності: a/b == c/d -> a*d == b*c
        is_equivalent = (user_improper_n * self.correct_d == self.correct_improper_n * user_d)

        if is_equivalent:
            # Для 5 класу вимагаємо правильний мішаний або скорочений дріб
    ...
    def _draw_overlapping_circles(self, ax, n, d, color):
        ...
        # Динамічний ліміт ширини, щоб кола не виходили за межі графіку
    ...
    def _build_solution_for_task(self):
        n1, d, n2 = self.task_n1, self.task_d1, self.task_n2
        w1, f_n1 = divmod(n1, d)
        w2, f_n2 = divmod(n2, d)
        ...
        self.solution_steps.append(
            ("normal", "Цілі частини додаємо до цілих, а дробові — до дробових.\nЗнаменник залишаємо без змін."))

        sum_w = w1 + w2
        sum_f_n = f_n1 + f_n2

        self.solution_steps.append(("bold", "--- КРОК 1: ДОДАВАННЯ ---"))
        self.solution_steps.append(("normal", f"1. Додаємо цілі частини: {w1} + {w2} = {sum_w}"))
        self.solution_steps.append(("normal", f"2. Додаємо чисельники: {f_n1} + {f_n2} = {sum_f_n}"))
        self.solution_steps.append(("normal", f"Знаменник залишається {d}."))

        if sum_w > 0:
            step_res = f"{sum_w} {sum_f_n}/{d}"
        else:
            step_res = f"{sum_f_n}/{d}"

        self.solution_steps.append(("normal", f"Отримуємо:\n-> {step_res}"))
        ...
        if final_n >= d:
            ...
            self.solution_steps.append(("normal",
                                        f"Дріб {final_n}/{d} є неправильним.\nВиділяємо цілу частину: {final_n} поділити на {d} = {extra_w} (цілих) і {rem_n} (остача)."))
            final_w += extra_w
            final_n = rem_n

            if final_n > 0:
                self.solution_steps.append(("normal",
                                            f"Додаємо до цілої частини: {sum_w} + {extra_w} = {final_w}.\n-> {final_w} {final_n}/{d}"))
            else:
                self.solution_steps.append(
                    ("normal", f"Додаємо до цілої частини: {sum_w} + {extra_w} = {final_w}.\n-> {final_w}"))

        gcd = math.gcd(final_n, d)
        if gcd > 1 and final_n > 0:
            self.solution_steps.append(("bold", "--- КРОК 3: СКОРОЧЕННЯ ДРОБУ ---"))
            reduced_n = final_n // gcd
            reduced_d = d // gcd
            self.solution_steps.append(
                ("normal", f"Чисельник і знаменник можна поділити на {gcd}.\n-> {reduced_n}/{reduced_d}"))
            final_n = reduced_n
            d = reduced_d

        if final_w > 0:
            if final_n > 0:
                self.solution_steps.append(("bold", f"Кінцева відповідь: {final_w} {final_n}/{d}"))
            else:
                self.solution_steps.append(("bold", f"Кінцева відповідь: {final_w}"))
        else:
            if final_n > 0:
                self.solution_steps.append(("bold", f"Кінцева відповідь: {final_n}/{d}"))
            else:
                self.solution_steps.append(("bold", f"Кінцева відповідь: 0"))
